...
import type { Player } from "../lib/arenaStore";
...
export default function Individual({ id, score, name, method, shapeIndex }: IndividualProps) {
  const snap = useSnapshot(arenaStore);
  const count = snap.scores[`player-${id}`] ?? score;
      ...
      <div className="w-full max-w-[180px] space-y-3 mt-2">
        ...
        <Button
          onClick={() => {
            ...
            if (arenaStore.scores[`player-${id}`] === undefined) {
              arenaStore.scores[`player-${id}`] = score;
            }
            arenaStore.submitPoints(`player-${id}`, points, message, date);
            method({ date, message });
            setPoints(0);
          }}
          variant={theme.btn}
